...
export function useUpdateThemeTabIcon() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, file }: { id: string; file: File }) => updateThemeTabIcon(id, file),
    onSuccess: (_, { id }) => {
    ...
    onError: (error: Error) =>
      toast.error(error.message || "Failed to upload tab icon"),
  })
}
