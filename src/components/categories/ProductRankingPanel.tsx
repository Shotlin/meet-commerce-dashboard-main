 ...
 * Both types support search-and-add (cross-listing a product into this
 ...
 *
 * The seed list comes from GET /categories/:id/products, which already
 * reflects the exact order customers currently see — dragging it and
 * saving IS setting the new order.
 */
...
import { getCategoryProducts } from "@/services/categoriesAdminService"
import { getProducts } from "@/services/products.service"
import type { ProductOption } from "@/services/catalogPickerService"
import { useSetCategoryProducts } from "@/hooks/useCategoriesAdmin"
...
export function ProductRankingPanel({ category, onClose }: ProductRankingPanelProps) {
  ...
  const isPrimary = (product: ProductOption) => !isBundle && product.category_id === category?.id
  ...
  const { data: seedProducts, isLoading } = useQuery({
    queryKey: ["categories-admin", "products", "seed", category?.id],
  ...
  const { data: searchResults, isFetching: isSearching } = useQuery({
    queryKey: ["categories-admin", "product-search", debouncedSearch],
