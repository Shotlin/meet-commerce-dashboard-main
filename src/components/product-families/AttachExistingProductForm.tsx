...
import { OptionEditorForm } from "./OptionEditorForm"
...
export function AttachExistingProductForm({ familyId, familyName, onAttached }: Props) {
  ...
  const handleSelectProduct = (product: AdminProduct) => {
    if (product.product_family_id === familyId) return
    if (product.product_family_id && product.product_family_id !== familyId) return
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {products.map((product) => {
              ...
              const inAnotherFamily = product.product_family_id && product.product_family_id !== familyId
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 border rounded-lg ${
                    canAttach ? "hover:bg-muted/50 cursor-pointer" : "opacity-60 cursor-not-allowed"
                  }`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      {product.option_label && (
                        <span className="text-xs text-muted-foreground">{product.option_label}</span>
                      )}
                      {product.net_quantity && !product.option_label && (
                        <span className="text-xs text-muted-foreground">{product.net_quantity}</span>
                      )}
                      {product.unit && <span className="text-xs text-muted-foreground">{product.unit}</span>}
                      <span className="text-xs font-medium">₹{product.sale_price ?? product.price}</span>
                    </div>
                  </div>
