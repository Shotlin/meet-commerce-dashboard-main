...
/**
 * Thin orchestration over the app's real coupon engine. "Create new" is a
 * single-target action only: a coupon's `code` is globally unique, so
 * looping a create across a bulk selection would collide on the 2nd call.
 * Bulk selections (`cartIds.length > 1`) can only assign an already-existing
 * coupon to every selected episode's user.
 */
