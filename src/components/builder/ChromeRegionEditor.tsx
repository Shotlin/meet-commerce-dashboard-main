...
// Some existing themes (this project's seed data included) were saved
// before categoryTabs existed on ThemeSections, so their theme_data is
// missing that key entirely — not just missing a field within it. Filling
// in only "when theme_data is null" (the old behavior) left those themes
// with a genuinely undefined sections.categoryTabs, which crashed
// RegionFields the moment that region was opened. Defaulting per-section
// (rather than all-or-nothing) makes every region safe to open regardless
// of how old or incomplete the saved theme_data is.
function withSectionDefaults(
  sections: Partial<ThemeSections> | null | undefined
): ThemeSections {
  const defaults = defaultThemeSections()
  return {
    topBar: sections?.topBar ?? defaults.topBar,
    storeSelector: sections?.storeSelector ?? defaults.storeSelector,
    categoryTabs: sections?.categoryTabs ?? defaults.categoryTabs,
    searchZone: sections?.searchZone ?? defaults.searchZone,
    bannerAnimation: sections?.bannerAnimation ?? defaults.bannerAnimation,
...
function normalizeThemeData(themeData: ThemeData | null | undefined): ThemeData {
  if (!themeData) {
    return {
  ...
  return { ...themeData, sections: withSectionDefaults(themeData.sections) }
}
...
export default function ChromeRegionEditor({
  ...
  const [draft, setDraft] = useState<ThemeData>(() =>
    normalizeThemeData(theme?.theme_data ? deepClone(theme.theme_data) : null)
  )

  useEffect(() => {
    setDraft(normalizeThemeData(theme?.theme_data ? deepClone(theme.theme_data) : null))
  }, [theme?.id, theme?.updated_at, theme?.theme_data])

  const isDirty = useMemo(() => {
    if (!theme) return false
    return JSON.stringify(normalizeThemeData(theme.theme_data)) !== JSON.stringify(draft)
  }, [theme, draft])
  ...
  const handleReset = () => {
    if (!theme) return
    const reset = normalizeThemeData(theme.theme_data)
    setDraft(reset)
    onDraftChange?.(reset)
  }
