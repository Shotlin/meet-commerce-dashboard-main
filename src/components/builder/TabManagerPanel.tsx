...
import { Archive, Check, GripVertical, ImagePlus, Loader2, Pencil, Plus, Star, X } from "lucide-react"
...
interface TabManagerPanelProps {
  ...
  onTabIconUpload?: (tabId: string, file: File) => Promise<unknown>
}
...
interface SortableTabItemProps {
  ...
  onSetDefault?: (tabId: string) => void
  onIconUpload?: (tabId: string, file: File) => Promise<unknown>
}
...
function SortableTabItem({ tab, isActive, onSelect, onUpdate, onArchive, onSetDefault, onIconUpload }: SortableTabItemProps) {
  ...
  const handleIconFile = useCallback(
    async (file: File) => {
      ...
      try {
        await onIconUpload(tab.id, file)
      } catch {
        // Surfaced to the user via the mutation's own error toast.
      } finally {
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        className="relative flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-slate-200 bg-slate-50 text-slate-400 hover:border-slate-300 hover:bg-slate-100"
        onClick={() => iconInputRef.current?.click()}
        aria-label="Upload tab icon"
      ...
      >
        {isUploadingIcon ? (
          ...
          <img src={tab.image_url} alt="" className="h-full w-full object-contain" />
        ) : (
